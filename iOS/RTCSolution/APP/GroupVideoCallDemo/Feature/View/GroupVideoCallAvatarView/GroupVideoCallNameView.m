...
@interface GroupVideoCallNameView ()
...
@implementation GroupVideoCallNameView
...
- (void)setMicStatus:(GroupVideoCallAvatarViewMicStatus)status {
    if (status == GroupVideoCallAvatarViewMicStatusOff) {
        self.micIcon.image = [UIImage imageNamed:@"room_mic_s" bundleName:HomeBundleName];
    } else if (status == GroupVideoCallAvatarViewMicStatusSpeaking) {
