...
/** 
     ...
     * @param  [in] volume Audio capture volume, the value range: [0,400] <br>
     *        + 0: Mute <br>
    ...
    */
     ...
     * @type api
     * @region screen sharing
     * @brief To adjust the local sound card capture volume. <br>
     *        After the local sound card acquisition is turned on, you can use this interface to adjust the capture volume.
     ...
     *        + Call stopAudioCapture{@link IRtcEngine#stopAudioCapture} to stop the internal audio capture. Otherwise, the internal audio capture will sustain until you destroy the engine instance. <br>
     *       +  To mute and unmute microphones, we recommend using unpublish{@link IRtcEngine#unpublish} and publish{@link IRtcEngine#publish}, other than stopAudioCapture{@link IRtcEngine#stopAudioCapture} and this API. Because starting and stopping capture devices often need some time waiting for the response of the device, that may lead to a short silence during the communication.
    ...
    */
     ...
     *                           This parameter describes the region to be shared after calling this interface, and the relative relationship between the 'source_info' setting area in startScreenVideoCapture{@link #IRtcEngine#startScreenVideoCapture}.
     ...
     * @type api
     * @region Audio management
     * @brief Designate the device as the current audio playback device. The default device is set via setDefaultAudioRoute{@link #IRtcEngine#setDefaultAudioRoute}. <br>
     *         When the audio playback route changes, you will get notified via onAudioRouteChanged{@link #IRtcEngineEventHandler#onAudioRouteChanged}.
     * @param [in] route Audio route. Refer to AudioRoute{@link #AudioRoute}. You cannot choose `kAudioRouteDefault`. <br>
     *        You cannot choose `kAudioRouteEarpiece` except in the communication audio mode.
     *        You can choose `kAudioRouteHeadsetBluetooth` or `kAudioRouteHeadsetUSB` except in the communication audio mode.
     * @return <br>
     *         + 0: Success <br>
     *         + < 0: Failure <br>
     * @notes <br>
     *          + Prior to call this API, call setAudioScenario{@link #IRtcEngine#setAudioScenario} and set the audio scenario to `kAudioScenarioTypeCommunication`.<br>
     *          + The audio route automatically switches to the wired or Bluetooth audio playback device once it is connected. <br>
     * Disconnecting the current device will have the audio route switched to the previous device. <br>
 ...
 *        + If the video capture is on, the setting will be effective once you call this API. If the video capture is off, the setting will be effective on when capture starts.
    ...
    */
     ...
     * @notes With the following APIs, you can set the audio data in the callbacks:
     *        enableAudioFrameCallback{@link #IRtcEngineLite#enableAudioFrameCallback},
    ...
    */
     ...
     * @type api
     * @region Video Management
     * @brief When video communication, SEI data is sent through video frames.
     ...
     * @notes   <br>
     *        + After calling this interface to log in, if you want to log out, you need to call logout{@link #IRtcEngine#logout}. <br>
     *        + You will receive onLoginResult{@link #IRtcEngineEventHandler#onLoginResult} after calling this API and log in successfully. But remote users will not receive notification about that.
     */
     ...
     * @notes  <br>
     *         + After collection, you also need to call publishScreen{@link #IRtcRoom#publishScreen} to collect the
     * screen audio Push to the far end. <br>
     *         + To turn off screen audio internal capture, call stopScreenAudioCapture{@link #IRtcEngine#stopScreenAudioCapture}.
     */
     ...
     * @notes To start the device audio recording, call startScreenAudioCapture{@link #IRtcEngine#startScreenAudioCapture}.
     */
     ...
     * @notes This method only controls the local audio The playinging state of the stream but does not affect the local audio playback device.
     */
     ...
     * @type api
     * @region  video management
     * @brief Set the path of a image, which is used to push to the far end after the user turns off the camera
     * @param file_path File path of the image. You can use the absolute path or the asset path(/assets/xx.png). The path should be less than 512 bytes. <br>
     *        The watermark image should be in BMP, PNG or JPG format.
     * @notes <br>
     *        + Set filePath empty to remove the image. <br>
     *        + You can only add one image to the mainstream. The newly added image replaces the previous one. <br>
     *        + You can call this API before and after joining room. <br>
     *        + the image will be published to the network and nerver displayed locally. <br>
     *        + When you enable simulcast mode, the image will be added to all video streams, and it will scale down to smaller encoding configurations accordingly. <br>
     */
  ...
  /** 
     ...
     *        + Once the local user subscribes to the stream of a remote user, the subscription to the remote user will sustain until the local user leaves the room or unsubscribe from it by calling unsubscribeStream{@link #RTCEngine#unsubscribeStream}.<br>
     ...
     *        + After calling this API, you will be informed of the calling result with onStreamSubscribed{@link #IRTCRoomEventHandler#onStreamSubscribed}.  <br>
     *        + Once the local user subscribes to the stream of a remote user, the subscription to the remote user will sustain until the local user leaves the room or unsubscribe from it by calling unsubscribeScreen{@link #RTCEngine#unsubscribeScreen}.<br>
     *        + Any other exceptions will be included in onStreamStateChanged{@link #IRTCRoomEventHandler#onStreamStateChanged}, see ErrorCode{@link #ErrorCode} for the reasons.
     */
    ...
    */
     ...
     *        Users within the same `appID` can call startPlayPublicStream{@link #IRtcEngine#startPlayPublicStream} to subscribe to the public stream regardless the user has joined which room or has not joined any room.
