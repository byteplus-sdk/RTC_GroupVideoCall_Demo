...
@interface GroupVideoCallLoginViewController () <UITextFieldDelegate>
...
@implementation GroupVideoCallLoginViewController
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)applicationBecomeActive {
    ...
    if ([topVC isKindOfClass:[GroupVideoCallLoginViewController class]]) {
...
- (void)jumpToRoomVC:(GroupVideoCallRoomUserModel *)localSession
    ...
    GroupVideoCallRoomViewController *roomVC = [[GroupVideoCallRoomViewController alloc] initWithVideoSession:localSession rtcToken:rtcToken duration:duration];
...
- (void)onClickEnableVideo:(BaseButton *)sender {
    if (sender.status != ButtonStatusIllegal) {
        ...
        [[GroupVideoCallRTCManager shareRtc] switchVideoCapture:isEnableVideo];
    } else {
        AlertActionModel *alertCancelModel = [[AlertActionModel alloc] init];
        alertCancelModel.title = LocalizedString(@"cancel");
        AlertActionModel *alertModel = [[AlertActionModel alloc] init];
...
- (void)dealloc {
    [[GroupVideoCallRTCManager shareRtc] disconnect];
